    ...
    def send_post_request(self, endpoint: str, data: dict[Miniature]):
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"✅ Object successfully created: {data['product_description[1][name]']} \n"
                      f"{url_create_object}")
            else:
                print(f"❌ Error occurred during object creation: {response.status_code} \n")
        except Exception as e:
                print(f"❌ Could not create an object:")
                print(f"❗️{str(e)}")
